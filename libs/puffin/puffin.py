...
class Puffin_(nn.Module):
    def __init__(self, use_cuda=False):

        super(Puffin_, self).__init__()
    ...
    def predict(self, seq_encode, seq_bp):
        ...
        if self.use_cuda:
            pred0 = self(torch.FloatTensor(seq_encode)[None, :, :].transpose(1, 2).cuda())
            pred0 = pred0.detach().cpu().numpy()
        else:
            pred0 = self(torch.FloatTensor(seq_encode)[None, :, :].transpose(1, 2))
