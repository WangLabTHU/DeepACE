...
class SeqNN:
    ...
    def build_block(self, current, block_params):
        ...
        if block_name[0].isupper():
            ...
            block_func = keras_func[block_name]
    ...
    def build_model(self, save_reprs: bool = True):
        ...
        for hi, head in enumerate(self.heads):
            ...
            if self.augment_rc:
                if self.preds_triu:
                    current = SwitchReverseTriu(self.diagonal_offset)(
                        [current, reverse_bool]
                    )
                else:
                    current = SwitchReverse(strand_pair)([current, reverse_bool])
    ...
    def build_ensemble(self, ensemble_rc: bool = False, ensemble_shifts=[0]):
        ...
        if ensemble_rc or shift_bool:
            ...
            if shift_bool:
                ...
                sequences = EnsembleShift(ensemble_shifts)(sequences)

            if ensemble_rc:
                # generate reverse complements and indicators
                sequences_rev = EnsembleReverseComplement()(sequences)
            else:
            ...
            if self.preds_triu:
                preds = [
                    SwitchReverseTriu(self.diagonal_offset)(
                        [self.model(seq), rp]
                    )
                ...
                preds = [
                    SwitchReverse(strand_pair)([self.model(seq), rp])
                    for (seq, rp) in sequences_rev
                ]

            # create layer
            # preds_avg = tf.keras.Average()(preds)
    ...
    def build_sad(self):
        ...
        # sum pool
        sad = preds_len * tf.keras.GlobalAveragePooling1D()(predictions)
    ...
    def evaluate(
        ...
        if loss_label == "bce":
            model.compile(
                ...
                loss=loss_fn,
                metrics=[
                    SeqAUC(curve="ROC", summarize=False),
                    SeqAUC(curve="PR", summarize=False),
                ],
            )
        else:
            model.compile(
                ...
                loss=loss_fn,
                metrics=[
                    PearsonR(num_targets, summarize=False),
                    R2(num_targets, summarize=False),
                ],
            )
