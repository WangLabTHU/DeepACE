...
mv /home/hyu/Digital_Platform/manuals/fig_dataset/control_Epigenetics_HepG2_neg /home/hyu/DeepACE/Preds/D04_deeptfbu
'''
...
def load_data(cell, motif=None):
    ...
    if dataset == "epigenetics":
        if pseudo_source == "negative":
            pseudo_data = np.load(f"./Preds/D04_deeptfbu/control_Epigenetics_{cell}_neg/uni_pred.npy")
    elif dataset == "MPRA":
        if pseudo_source == "negative":
            pseudo_data = np.load(f"./Preds/D06_mpra/control_MPRA_{cell}_neg/uni_pred.npy")
        elif pseudo_source == "positive":
            pseudo_data = np.load(f"./Preds/D06_mpra/control_MPRA_{cell}_pos/uni_pred.npy")
    if pseudo_source == "random":  # random
...
''' Dataset Preparation for negpos '''

mode = "pca50"
metric_type = "mahalanobis"
output_base = "./Supps/S18_robust_negpos/results_negpos"

for pseudo_source in ["negative", "positive"]:
    for dataset in ["MPRA", "epigenetics"]: #  
        if dataset == "MPRA":            
            cells = ["HepG2", "K562", "SKNSH"]
        elif dataset == "epigenetics":
            if pseudo_source == "positive":
                continue
            else:
        ...
        for i, cell in enumerate(cells):
            ...
            output_dir = f"{output_base}/anchors_{pseudo_source}"
            os.makedirs(output_dir, exist_ok=True)
            analyze_pseudo_similarity(sample_data, sample_labels, sorted_labels, plot_tag, 
                                      metric=metric_type, output_dir=output_dir)


''' Radar Preparation for negpos '''
...
for pseudo_source in ["negative", "positive"]:
    ...
    all_labels = []
    output_dir = f"./Supps/S18_robust_negpos/results_negpos/anchors_{pseudo_source}/"
    for dataset in ["MPRA", "epigenetics"]: #  
        if dataset == "MPRA":            
            cells = ["HepG2", "K562", "SKNSH"]
        elif dataset == "epigenetics":
...
save_dir = "./Supps/S18_robust_negpos/"
for pseudo_source in ["negative", "positive"]:
    file_map = {
            "Filter_Efficiency": f"./Supps/S18_robust_negpos/results_negpos/anchors_{pseudo_source}/radar_pseudo_effect.csv",
            "Retained_Activity": f"./Supps/S18_robust_negpos/results_negpos/anchors_{pseudo_source}/radar_screen_effect.csv"
        }
    for metric_name, path in file_map.items():
        ...
        try:
            for i, cat in enumerate(categories):
                ...
                sns.despine()
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, f"lineplot_{pseudo_source}_{metric_name}.pdf"), bbox_inches='tight')
            plt.close()
        except:
            continue
