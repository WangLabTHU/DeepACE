...
/home/hyu/Digital_Platform/manuals/figs2_virtual_screen_compare.py
/home/hyu/Digital_Platform/manuals/figs3_virtual_screen_negpos.py
/home/hyu/Digital_Platform/manuals/figs5_virtual_screen_markov_radar.py
...
def load_data(cell, motif=None, mode="total"):
    ...
    if mode == "total":
        ...
        uni_selected = PCA(n_components=100, random_state=42).fit_transform(combined_data)
    elif mode == "xfilt":
        uni_selected, _ = get_matched(combined_data, anno_df, [match_tag], top_cols=None, match_mode="soft")
    elif mode == "efilt":  # standard mode
        if cell == "SKNSH":
            uni_selected, _ = get_matched(combined_data, anno_df, [match_tag, "SH-SY5Y", "SKNSH"], top_cols=None, match_mode="soft")
        elif cell == "WTC11":
            uni_selected, _ = get_matched(combined_data, anno_df, [match_tag, "iPSC"], top_cols=None, match_mode="soft")
        elif dataset == "epigenetics" and motif == "ELF1_1_aim":
            uni_selected, _ = get_matched(combined_data, anno_df, ["ELF1", "HepG2"], top_cols=None, match_mode="soft")
        elif dataset == "epigenetics" and motif != "ELF1_1_aim":
            uni_selected, _ = get_matched(combined_data, anno_df, ["HNF", "HepG2"], top_cols=None, match_mode="soft")
        else:
            uni_selected, _ = get_matched(combined_data, anno_df, [match_tag], top_cols=None, match_mode="soft")
    else:
        raise ValueError("Invalid mode input! Choose from 'total', 'pca50', 'pca100', 'xfilt', 'efilt'.")
...
''' Dataset Preparation for algos '''

pseudo_source = "random"
metric_type = "mahalanobis"
output_base = "./Supps/S17_robust_algos/results_algos"
...
for dataset in ["MPRA", "epigenetics"]: 
    if dataset == "MPRA":
        ...
        raise ValueError("Invalid dataset input!")
    for mode in ["efilt", "xfilt", "total", "pca100"]: 
            for i, cell in enumerate(cells):
                ...
                output_dir = f"{output_base}/algo_{mode}"
...
for mode in ["efilt", "xfilt", "total", "pca100"]: 
    ...
    all_labels = []
    output_dir = f"./Supps/S17_robust_algos/results_algos/algo_{mode}/"
    for dataset in ["MPRA", "epigenetics"]: 
        if dataset == "MPRA":
            cells = ["HepG2", "K562", "SKNSH"]
        elif dataset == "epigenetics":
...
save_dir = "./Supps/S17_robust_algos/"
for mode in ["efilt", "xfilt", "total", "pca100"]: 
    file_map = {
            "Filter_Efficiency": f"./Supps/S17_robust_algos/results_algos/algo_{mode}/radar_pseudo_effect.csv",
            "Retained_Activity": f"./Supps/S17_robust_algos/results_algos/algo_{mode}/radar_screen_effect.csv"
        }
    for metric_name, path in file_map.items():
        ...
        for i, cat in enumerate(categories):
            ...
            sns.despine()
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"lineplot_{mode}_{metric_name}.pdf"), bbox_inches='tight')
        plt.close()
