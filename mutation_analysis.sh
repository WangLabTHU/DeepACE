...
#
# This script generates a Python script for mutation analysis and executes it.
# It processes sequence datasets, filters predictions, and generates a differential heatmap.
#
# Usage:
#   ./mutation_analysis.sh --pos_pred <positive_pred_path> --neg_pred <negative_pred_path> \
#                         --outs_path <output_path> --csv_path <tracks_csv> \
#                         --mode <match_mode> --keywords <keywords> \
#                         [--top_rows <rows>] [--top_cols <cols>]
#
# Example:
#   ./mutation_analysis.sh --pos_pred /path/to/pos_pred.npy \
#                         --neg_pred /path/to/neg_pred.npy --outs_path /path/to/output \
#                         --csv_path /path/to/tracks.csv --mode hard \
#                         --keywords '{"celltype": ["HepG2", "K562"], "motif": ["BHLHE40"]}' \
#                         --top_rows 10 --top_cols 30
#   ./mutation_analysis.sh --pos_pred /path/to/pos_pred.npy \
#                         --neg_pred /path/to/neg_pred.npy --outs_path /path/to/output \
#                         --csv_path /path/to/tracks.csv --mode soft \
#                         --keywords '["BHLHE40"]' --top_rows 10 --top_cols 30
# ----------------------------------------------------------------------------

# Function to display usage
usage() {
  echo "Usage: $0 --pos_pred <positive_pred_path> --neg_pred <negative_pred_path> --outs_path <output_path> --csv_path <tracks_csv> --mode <match_mode> --keywords <keywords> [--top_rows <rows>] [--top_cols <cols>]"
  echo "  For --mode soft, --keywords should be a JSON list, e.g., '[\"BHLHE40\"]'"
  echo "  For --mode hard, --keywords should be a JSON dict, e.g., '{\"celltype\": [\"HepG2\", \"K562\"], \"motif\": [\"BHLHE40\"]}'"
  exit 1
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --pos_pred) POS_PRED="$2"; shift 2 ;;
    --neg_pred) NEG_PRED="$2"; shift 2 ;;
    --outs_path) OUTS_PATH="$2"; shift 2 ;;
    --csv_path) CSV_PATH="$2"; shift 2 ;;
    --mode) LOC="$2"; shift 2 ;;
    --keywords) KEYWORDS="$2"; shift 2 ;;
    --top_rows) TOP_ROWS="$2"; shift 2 ;;
    --top_cols) TOP_COLS="$2"; shift 2 ;;
    *) echo "[ERROR] Unknown argument: $1"; usage ;;
  esac
done

# Validate required arguments
[[ -z "${POS_PRED:-}" ]] && { echo "[ERROR] --pos_pred is required"; usage; }
[[ -z "${NEG_PRED:-}" ]] && { echo "[ERROR] --neg_pred is required"; usage; }
[[ -z "${OUTS_PATH:-}" ]] && { echo "[ERROR] --outs_path is required"; usage; }
[[ -z "${CSV_PATH:-}" ]] && { echo "[ERROR] --csv_path is required"; usage; }
[[ -z "${LOC:-}" ]] && { echo "[ERROR] --mode is required"; usage; }
[[ -z "${KEYWORDS:-}" ]] && { echo "[ERROR] --keywords is required"; usage; }

# Validate match_mode
if [[ "$LOC" != "soft" && "$LOC" != "hard" ]]; then
  echo "[ERROR] --mode must be 'soft' or 'hard'"
  usage
fi

# Set default values for optional arguments
TOP_ROWS="${TOP_ROWS:-None}"
TOP_COLS="${TOP_COLS:-30}"

# Set project directory and report file
PROJECT_DIR="$(pwd)"
REPORT_FILE="${OUTS_PATH}/reports_mutation_analysis.log"
SCRIPT="${OUTS_PATH}/script_mutation_analysis.py"

# Calculate BASE_DIR based on the script's location
...
generate_py() {
  local out_path="$1"
  local py_script="${out_path}/script_mutation_analysis.py"
...
EOF
  ...
  echo "[INFO] Generated Python script: $py_script"
}

# Generate Python script
generate_py "$OUTS_PATH"
...
echo "[INFO] Running mutation analysis in env Digital_Platform_lightning" | tee -a "$REPORT_FILE"
...
echo "[INFO] Mutation analysis completed"